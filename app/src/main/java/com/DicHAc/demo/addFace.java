...
public class addFace extends AppCompatActivity implements SurfaceHolder.Callback {
    ...
    public void addF(User user,String UserId){
       // myRef.child(UserId).setValue(user);
    }
    /* Triggered by pressing the add button, possible: 1 added successfully, 2 added repeatedly, 3 no face found, 4 no id entered */
    //write here to add to firebase
    public void addEmb(View view) {
        if (readyFace) {
           ...
           Intent intent = new Intent(addFace.this,confirm_add.class);
            intent.putExtra("fullname",addName);
            intent.putExtra("embSet",embSet);
