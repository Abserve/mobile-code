...
public class MainActivity extends AppCompatActivity implements SurfaceHolder.Callback {
    ...
    private int[] landmarks = new int[10];  // Key points for storing faces
             ...
             public boolean onNavigationItemSelected(@NonNull MenuItem item) {
                 switch (item.getItemId()) {
                     case R.id.addMenu:
                         ...
                         intentAdd.putExtra("id",getIntent().getStringExtra("id"));
                         intentAdd.putExtra("ac",getIntent().getStringExtra("ac"));
                     ...
                     //    return true;
                     case R.id.ppp:
                         Intent ppp = new Intent(MainActivity.this, PointedActivity.class);
                         Bundle args = new Bundle();
                         args.putSerializable("ARRAYLIST",(Serializable)list);
                         ppp.putExtra("mylist", args);
                         ppp.putExtra("idop",getIntent().getStringExtra("id"));
                         ppp.putExtra("ac",getIntent().getStringExtra("ac"));
                         startActivity(ppp);
                         return true;
                  }
                 return true;
    ...
    protected void onDestroy() {
        ...
        Toast.makeText(MainActivity.this, list.get(hh),
            ...
            public void analyze(@NonNull ImageProxy image) {
                ...
                if (faceInfo.length > 1) {
                    ...
                    for (int i = 0; i < faceInfo[0]; i++) {
                        ...
                        face = Bitmap.createBitmap(faceBMP, bbox[0], bbox[1], bbox[2] - bbox[0], bbox[3] - bbox[1]);  // Image is cropped by bbox
                        ...
                        if (guessID != null) {
                            guessResult.put(guessID, new int[]{bbox[2], bbox[3]});
                             // face is recognized cromba
                            list.add(guessID);
                            // editorOp.putString("name",guessID);
                        }
    ...
    private void showMultiChoiceDialog() {
        ...
        // Set the options that are selected by default, all of them are false and none of them are selected by default
  ...
  /*  private void DelFace(String item) {
        DatabaseReference ref = FirebaseDatabase.getInstance().getReference();
        Query applesQuery = ref.child("users").orderByChild("name").equalTo(item);
   ...
   */
    ...
    private String guess(float[] emb, Map<String, String> embs) {
        ...
        if (ids.isEmpty()) {
            return null;  // Not found in the database return empty
        } else {
